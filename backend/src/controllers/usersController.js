...
const getUsers = async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT id, username, full_name, email, role, is_active, created_at FROM users ORDER BY role, full_name'
    );
    res.json(rows);
  } catch (err) {
...
const createUser = async (req, res) => {
  ...
  try {
    ...
    if (err.code === '23505') return res.status(409).json({ error: 'Username o email ya existe' });
...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    values.push(id);
    const { rows } = await pool.query(
      `UPDATE users SET ${fields.join(',')} WHERE id=$${i} RETURNING id, username, full_name, email, role, is_active`,
      values
    );
    if (!rows.length) return res.status(404).json({ error: 'Usuario no encontrado' });
...
const getUserPasswordPlain = async (req, res) => {
  // HOST only - returns masked info (we don't store plain passwords; just confirm existence)
  res.json({ message: 'Las contraseñas están encriptadas. Use la función de reset.' });
};

module.exports = { getUsers, getUserById, createUser, updateUser, deleteUser, getUserPasswordPlain };
